...
ASE4 = mean((Post_Vaccine$new_deaths_per_million[92:96] - preds4[1:5])^2)
ASE4 
...
preds5 = predict(VaccineVar1, n.ahead = 5)

ASE5 = mean((Post_Vaccine$new_deaths_per_million[92:96] - preds5$fcst$y2)^2)
ASE5 #13.43509
```
...
preds6 = predict(VaccineVar2, n.ahead = 5)

ASE6 = mean((Post_Vaccine$new_deaths_per_million[92:96] - preds6$fcst$y2)^2)
ASE6 #11.37751
```


#### VAR MODEL 3 NEEDS ATTENTION ####


```{r pressure, echo=FALSE}
time = data.frame(time = seq(1,91,1))
VaccineVar3 = VAR(cbind(Post_Vaccine2$new_cases_per_million, Post_Vaccine2$new_deaths_per_million, Post_Vaccine2$icu_patients_per_million,Post_Vaccine2$hosp_patients_per_million, Post_Vaccine2$new_vaccinations_smoothed_per_million), type = 'const', lag.max = 5, exogen = time)
AIC(VaccineVar3) #3084.604

preds7 = predict(VaccineVar3, n.ahead = 5) # NEEDS ATTENTION : Not Forecasting
...
library(vars)
library(tswge)
Vacc_Train = Post_Vaccine[1:91,]
Vacc_Test = Post_Vaccine[92:96,]
Vacc_xreg = data.frame(Vaccine = ts(Vacc_Train$new_vaccinations_smoothed_per_million))
set.seed(2)
Vacc_fit.mlp1 = mlp(ts(Vacc_Train$new_deaths_per_million) ,reps = 50,comb = "mean", xreg = Vacc_xreg ) #combine all 50 forcast
Vacc_fit.mlp1 # 5 hnode #mse 1.11
plot(Vacc_fit.mlp1)
fore_vacc_df_exp = data.frame(Vaccine = ts(Post_Vaccine$new_vaccinations_smoothed_per_million))
Vacc_fore.mlp1 = forecast(Vacc_fit.mlp1, h = 5, xreg = fore_vacc_df) 
plot(Vacc_fore.mlp1)
...
allexp_fore.m2p1 = forecast(exp_fit.mlp1, h = 5, xreg = fore_all_exp_df) 
